...
# 2.1.1 Initial quality assessment
...
# 2.1.2 External to R data opperations
...
In the previous section, we have identified promoters overlaping a H3K27ac peak (the pos.TSS object). In order to get a comprehensive view of the distribution of H3K27ac around the corresponding TSS, we extend the analysed region to ±1000
1000
 bp around the TSS. We divide each of these 2000 bp regions into 20 bins of 100 bp length each and order the bins with increasing position for genes on the ’+’ strand and decreasing for genes on the ’-’ strand.
...
It should appear...
The problem is that my window is to small, I should open a new file (graphics)...
